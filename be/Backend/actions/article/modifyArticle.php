<?php
	...
	if (isset($_GET['id']))
	{
		header("location:../../admin/article/index.php?id=".$_GET['id']);
		$count++;
	}

	if (isset($_POST['content']) && isset($_POST['titre']))
	{
		$count++;
...
############# preparation des information###############################################################################################
 ...
 ############# ########################################################################################################
		...
		$result=$article->saveOrUpdate($_POST['id']);
	...
	if ($result)
		...
		{
			header("location:../../admin/article/index?c=failed&&nbr=".$permissionok."&&info=$result");
		}

	}
##########################################################################################################

    # tester si ona dans un cas d'intrusion
	if ($count==0)
	{
